...
- **Note**: Access to an eBay production keyset was not possible for the scope of this project. As a consequence, the dataset created for this project had was crafted from items on eBay's Sandbox Environment (using a sandbox key for API calls). This hinders the programs ability to retrieve useful outputs in most situations. Firstly, because Sandox Environment mostly consists of low-quality test item postings. Secondly, because of the small quantity of postings, a dataset of only around 1,500 unique items could be collected. With that aside, this project showcases that if given a production keyset to scrape high quality data (around 20,000 diverse listing would be needed), the program would be able to carry out its goals. More details about this are noted in `DataScraper/README.md`
